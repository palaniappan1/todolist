...
@AndroidEntryPoint
class AddEditTaskFragment : Fragment(R.layout.fragment_add_task) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launchWhenStarted {
            viewModel.addEditEvent.collect { event ->
                when (event) {
                    is AddEditTaskViewModel.AddEditTaskEvent.ShowInvalidInputMethod -> {
                        Snackbar.make(requireView(), event.errorMessage, Snackbar.LENGTH_LONG)
                            .show()
                    }
